...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        die("Error: CSRF token inválido: no es igual");
    }
    
    // Eliminar el token CSRF solo después de la validación y procesamiento del formulario
    unset($_SESSION['csrf_token']); // Elimina el token CSRF de la sesión

    // Regenera un nuevo token CSRF para la siguiente carga de página
...
<body>
    ...
    <div class="container">
        <!-- Formulario -->
        <form id="item_add_form" action="add_item.php" method="post" onsubmit="return validarMatricula();">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'], ENT_QUOTES, 'UTF-8') ?>">
            <label for="nMatricula">Matrícula:</label>
            <input type="text" id="nMatricula" name="nMatricula" value="<?= htmlspecialchars(strip_tags($nMatricula), ENT_QUOTES, 'UTF-8') ?>" required>
            <label for="marcamodelo">Marca y modelo:</label>
            <input type="text" id="marcamodelo" name="marcamodelo" value="<?= htmlspecialchars(strip_tags($marcamodelo), ENT_QUOTES, 'UTF-8') ?>" required>
            <label for="color">Color:</label>
            <input type="text" id="color" name="color" value="<?= htmlspecialchars(strip_tags($color), ENT_QUOTES, 'UTF-8') ?>" required>
            <label for="kms">Kilómetros:</label>
            <input type="number" id="kms" name="kms" value="<?= htmlspecialchars(strip_tags($kms), ENT_QUOTES, 'UTF-8') ?>" required>
            <label for="cv">Caballos:</label>
            <input type="number" id="cv" name="cv" value="<?= htmlspecialchars(strip_tags($cv), ENT_QUOTES, 'UTF-8') ?>" required>
            <label for="anio">Año:</label>
            <input type="number" id="anio" name="anio" value="<?= htmlspecialchars(strip_tags($anio), ENT_QUOTES, 'UTF-8') ?>" required>
            <button id="item_add_submit" name="item_add_submit" type="submit">Agregar Coche</button>
        </form>
    </div>
    <nav>
